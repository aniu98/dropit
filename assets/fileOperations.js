...
function performFileOperation(file, operation) {
    const operationName = operationMap[operation];
    if (!operationName) {
        throw new Error(`未定义的操作键: ${operation}`);
    }
...
function autoPerformFileOperation(file, operationKey) {
    return performFileOperation(file, operationKey);
}
// // 使用 autoPerformFileOperation 函数
autoPerformFileOperation('example.txt', '$0');